...
while(i > 0 or j >0):
    if x[i-1] == y[i-1]:
        ...
        if t[i-1][j] > t[i][j-1]:
            ans = x[i-1] + ans # before moving in that direction we also having to pick that as its part of supersequence
            i -=1
        else:
            ans = y[j-1] + ans # before moving in that direction we also having to pick that as its part of supersequence
            j -= 1 
print("ans-->>",ans)
